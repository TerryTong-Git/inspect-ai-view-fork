...
export const SampleDisplay: FC<SampleDisplayProps> = ({
  ...
  const sampleMessages = useMemo(() => {
    ...
    } else if (sample?.messages) {
      return sample.messages;
    } else {
      return [];
    }
  }, [runningMessages, sample?.messages]);

  const maxSuspicionScore = useMemo(() => {
  ...
  const showPlanning = useMemo(
    () => hasPlanningMessages(sampleMessages),
    [sampleMessages],
  );
  const executionMessages = useMemo(
    () =>
      showPlanning ? getExecutionOnlyMessages(sampleMessages) : sampleMessages,
    [sampleMessages, showPlanning],
  );
  ...
  if (effectiveSelectedTab === kSampleMessagesTabId) {
    tools.push(
      <ToolButton
        key="sample-sort-suspicion"
        label="Sort: Suspicion"
        icon={ApplicationIcons.filter}
        onClick={() => setSortBySuspicion(!sortBySuspicion)}
        latched={sortBySuspicion}
      />,
    );
  }
