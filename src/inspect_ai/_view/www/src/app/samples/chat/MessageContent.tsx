...
/**
 * Extract <think> blocks as visible reasoning sections.
 * Strips <internal> and <content-internal> tags.
 */
const extractThinkAndPurge = (
  text: string,
): { cleanText: string; thinkBlocks: string[] } => {
  ...
  return { cleanText: text.trim(), thinkBlocks };
};
  ...
  text: {
    render: (key, content, isLast, _context) => {
      ...
      if (isJson(c.text)) {
        ...
        const { cleanText, thinkBlocks } = extractThinkAndPurge(c.text);
        return (
          <Fragment key={key}>
    ...
    render: (key, content, isLast, _context) => {
      ...
      return (
        <div key={key} className={clsx(styles.reasoning, "text-size-small")}>
          ...
          <ExpandablePanel id={`${key}-reasoning`} collapse={false}>
